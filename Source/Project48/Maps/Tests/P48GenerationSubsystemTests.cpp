...
bool FP48GenerationContextTest::RunTest(const FString& Parameters)
{
	...
	Snapshot.Seed = 48123;
	Snapshot.Revision = 7;
	Snapshot.RequiredPlayerCount = 3;
	...
	TestEqual(TEXT("Player count belongs to the snapshot"), Context.RequiredPlayerCount, 3);
	TestTrue(TEXT("Context is valid only with generation and player count"), Context.IsValid());
